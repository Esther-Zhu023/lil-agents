...
interface WalkCycleFrame {
  bodyOffsetY: number;
  legOffsetY: number;
  armOffsetY: number;
}

type WalkCycle = WalkCycleFrame[];
...
const WALK_CYCLE_FRAMES: WalkCycle = [
...
function drawCharacter(char: Character, walkFrame: WalkCycleFrame): void {
...
function updateCharacters(deltaTime: number): void {
...
// Listen for character AI changes from main process (tray menu)
declare global {
  interface Window {
    electronAPI?: {
      setCharacterAI: (characterId: number, provider: string) => void;
      getCharacters: () => Promise<Array<{ id: number; name: string; aiProvider: string }>>;
      onSetCharacterAI: (callback: (data: { characterId: number; provider: string }) => void) => void;
    };
  }
}
...
(window as any).overlayAPI = {
  setCharacterAI: (charId: number, provider: string) => {
